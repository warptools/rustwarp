// the quick helpers section -->

pub fn deserialize_str<'de, T>(data: &'de str) -> Result<T, serde_yml::Error>
where
	T: serde::Deserialize<'de>,
{
	let deserializer = serde_yml::Deserializer::from_str(data);
	serde_yml::with::singleton_map_recursive::deserialize(deserializer)
}
...
// the massive copyfork of serializer, with reduced/redirected functionality -->

use serde::ser;
use serde_yml::libyml::emitter::{Emitter, Event, Mapping, Scalar, ScalarStyle, Sequence};
use serde_yml::modules::error::{self, Error, ErrorImpl};
use serde_yml::value::tagged::{self, MaybeTag};
use std::fmt::Display;
	...
	W: io::Write,
{
	...
	/// Emit a scalar value.
	pub fn emit_scalar(&mut self, mut scalar: Scalar<'_>) -> Result<()> {
	...
	// Used by parts of ser::Serializer as an alternative to `serialize_str` when there is a hint that the string might want less quoting
	// (namely, that it's a struct field name, enum member name, etc).
	fn serialize_str_bare(&mut self, value: &str) -> Result<()> {
		self.emit_scalar(Scalar {
			tag: None,
			value,
			style: ScalarStyle::Plain, // or ScalarStyle::Any ?
		})
	}
}
	...
	W: io::Write,
{
	...
	fn serialize_str(self, value: &str) -> Result<()> {
		// CHANGED: throw away massive amounts of overly fancy code about inferring necessity of quotes.
		// Our logic is: yes, quotes.  Unless it's multiline, and then, that.
	...
	fn serialize_unit_variant(
		...
		self.serialize_str_bare(variant)
	}
