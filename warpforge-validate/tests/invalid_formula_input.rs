...
#[test]
fn mount_invalid_format() {
	let formula = r#"
		{
			"formula": {
				"formula.v1": {
					"inputs": <missing_root>{
						"/path/1": <mount_invalid_format>"mount"</mount_invalid_format>,
						"/path/2": <mount_invalid_format>"mount:ro"</mount_invalid_format>,
						"/path/3": "mount:ro:/host/path"
					}</missing_root>,
					"action": "echo",
					"outputs": {}
				}
...
#[test]
fn mount_invalid_type() {
	let formula = r#"
		{
			"formula": {
				"formula.v1": {
					"inputs": <missing_root>{
						"/path": <mount_invalid_type>"mount:rx:/host/path"</mount_invalid_type>
					}</missing_root>,
					"action": "echo",
					"outputs": {}
				}
