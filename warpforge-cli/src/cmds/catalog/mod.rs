...
pub struct ReadItemCmdArgs {
    ...
    #[arg(value_parser = warpforge_api::catalog::CatalogRef::from_str)] // This seems "close" but I would love to know why it wants the error to be `Box<dyn Error + Send + Sync + 'static>`.  That's a lot of constraints, and especially the lifetime one makes no sense to me.
